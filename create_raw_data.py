...
def clean_data(df,position_dict,keep_cols):
...
if __name__ == '__main__':
    ...
    historic_data = collect_data(MIN_YEAR,MAX_YEAR,RAW_COLUMNS)

    # subset and clean position ids
    keep_cols = IDENTIFIERS+REALS+CATEGORIES+TARGET
    historic_data = clean_data(historic_data,POS_IDS,keep_cols)

    #Write to file
    file_dir = './data/raw/raw_df.csv'
    historic_data.to_csv(file_dir)
    print("Saving raw file: %s" % file_dir)
    mlflow.log_artifact(file_dir, "raw-csv-dir")
